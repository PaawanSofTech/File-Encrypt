...
const Documents = ({docs,contract,name,val,setfolderclosed}) => {
  ...
  return (
    ...
    }
    {
      fileName.length === 0 
      ? 
      <button onClick={sendImage} className='fixed bottom-16 bg-[#0e3851] rounded-full px-9 py-[4px] text-[1.8rem] '> Upload Document</button>
      :
      <button onClick={uploadImage}  className='fixed bottom-16 flex flex-row  bg-[#0e3851] items-center rounded-full px-9 py-[4px]'>
